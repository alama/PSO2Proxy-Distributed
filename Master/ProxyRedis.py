...
def plugin_handler(message):
    print(
        (
            "[PLUGINMSG] Got plugin message on channel {}: {}".format
            (
                message['channel'],
                message['data']
            )
...
def servercomm_handler(message):
    ...
    if 'command' in msgobj and msgobj['command'] in CommandHandlers:
        ...
        print(
            (
                "[SERVERCOMM] Got unknown data on channel {}: {}".format
                (
                    message['channel'],
                    message['data']
                )
...
if redis_config['db_pass'] == '':
    r = redis.StrictRedis(
        host=redis_config['db_host'],
        port=redis_config['db_port'],
        db=redis_config['db_id']
    )
else:
    r = redis.StrictRedis(
        host=redis_config['db_host'],
